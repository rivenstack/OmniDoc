...
item as "passed = RTL ships"; RTL items pass when the _discipline_ is
...
| 5.2  | Ask-with-empty-corpus explains _why_ it cannot answer and offers a next step (capture/import)                                      | Ask               | Not a generic error; explains the actual cause                                            |
...
## 9. Dual-mode, mode honesty, and corpus labelling (extended by D-01)

Added 2026-09-16 from the D-01 design package
(`docs/design/dual-mode/`, `docs/design/states/sample-vs-mine.md`).
These are **additional gate checks**, not replacements for §1–§8.

| #    | Check                                                                                                               | Journey       | Pass criteria                                                                     |
| ---- | ------------------------------------------------------------------------------------------------------------------- | ------------- | --------------------------------------------------------------------------------- |
| 9.1  | Runtime mode is always visible near Ask (Mock / operator-funded demo / your key)                                    | Ask           | Mode chip present without opening settings; matches the resolved mode             |
| 9.2  | Corpus ownership (Sample vs Mine) is labelled on lists, Ask scope, answer header, and citations                     | Retrieve, Ask | Label present on every surface; driven by API, not inferred client-side           |
| 9.3  | `no_supported_answer` / `refused_policy` are visually distinct from transport errors                                | Ask           | Refusal uses neutral/info treatment, never danger styling                         |
| 9.4  | Failure copy names the failing mode and states that mock answers still work                                         | Ask, Settings | Present whenever only a live path failed                                          |
| 9.5  | No silent fallback between operator free-tier and customer key                                                      | Ask           | Any payer change is explicit, confirmed, and reflected in mode chip + usage strip |
| 9.6  | Usage `unavailable` renders as an honest neutral state, not an error and not fabricated figures                     | Ask, Settings | "Usage not available from provider" styled neutral/info; no invented currency     |
| 9.7  | Cookbook key handling is write-once: no full key shown after save; masked prefix only                               | Settings      | Re-entry required to replace; no key characters in any live region or log         |
| 9.8  | Verify result is announced as status without key characters, and verify failure is distinguishable from Ask failure | Settings, Ask | Separate copy/states for the two                                                  |
| 9.9  | No UI copy implies zero retention / ZDR while ~30-day abuse-log retention is policy                                 | All           | Absent ZDR/never-log claims; factual retention line only                          |
| 9.10 | No live-AI-first first-run path; first value is reachable on the mock/sample path                                   | Onboarding    | No provider key required before first cited Ask or honest refusal                 |

## 10. Enterprise-scale and commerce honesty (extended by D-01)

Added 2026-09-16 from REC-15/REC-18 and the portfolio guardrails.

| #    | Check                                                                          | Journey | Pass criteria                                                                  |
| ---- | ------------------------------------------------------------------------------ | ------- | ------------------------------------------------------------------------------ |
| 10.1 | Workspace/team chrome is honest at n≈1–few                                     | All     | Real memberships only; no fake org charts, seat matrices, or directory theatre |
| 10.2 | Sample workspace is clearly non-production and separated from "Mine"           | All     | Clearly labelled; not presented as an enterprise tenant                        |
| 10.3 | No enterprise billing / invoice / seat / tax UI                                | All     | Absent (portfolio scale)                                                       |
| 10.4 | No fabricated accuracy badges, testimonials, user counts, or conversion claims | All     | Absent                                                                         |
| 10.5 | No commerce/checkout residue                                                   | All     | Absent                                                                         |

## 11. Design-spec conformance (D-01)

Added 2026-09-16. Verify the built UI against the D-01 package rather
than against a description of it.

| #    | Check                                                                                         | Pass criteria                                                                                                                 |
| ---- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 11.1 | Tokens consumed from the design token contract                                                | Components use semantic tokens; no hard-coded colors/spacing bypassing the contract                                           |
| 11.2 | Single `lang`/`dir` source with Base UI `Direction`                                           | One place sets `lang`/`dir`; no per-component `dir` overrides                                                                 |
| 11.3 | Logical CSS used for flippable layout                                                         | Physical `left`/`right`/`margin-left` absent except documented true exceptions                                                |
| 11.4 | `bdi`-equivalent isolation on identifiers, code tokens, URLs, key prefixes, usage/request IDs | Present in markup; verifiable by inspection (no RTL locale needed)                                                            |
| 11.5 | Component states exist as Storybook stories                                                   | Every state listed in `docs/design/components/inventory.md` is represented, with reduced-motion variants where motion applies |
| 11.6 | Loading skeletons are static under reduced motion                                             | No shimmer sweep under `prefers-reduced-motion: reduce`                                                                       |
| 11.7 | Empty / error / refusal / indexing states match the D-01 inventory                            | Copy and treatment match `docs/design/states/`; one primary CTA per empty state                                               |
...
- `docs/frontend/README.md` §"Stack-independent non-negotiables" items 4
  and 5
- **D-01 design package** (`docs/design/`) — §9–§11 above were added
  2026-09-16 from the design system, journey specs, dual-mode chrome, and
  state inventory. The design package extends this checklist; it does not
  replace or weaken §1–§8.
