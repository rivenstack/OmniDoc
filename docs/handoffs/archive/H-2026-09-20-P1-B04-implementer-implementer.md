...
**Completed 2026-09-20.** B-04 now provides the local-profile JDBC
`NotesPort` adapter and the frozen S-02 notes HTTP surface. Production AI,
ingestion, pgvector retrieval, mocks, and Phase Check remain unopened.

### Delivered

1. `JdbcNotesAdapter` runs every statement inside a Spring transaction with
   actor + tenant GUCs on the same connection, and independently checks
   workspace membership.
2. Create/update maintain append-only `note_versions`; update uses a database
   compare-and-swap on `current_version_id`, so two concurrent writers yield
   exactly one success and one `conflict`.
3. List/get/update hide soft-deleted notes. Purge intentionally resolves
   tombstones, locks the note, and removes the note plus all versions.
4. Keyset pagination is ordered by `updated_at DESC, note_id DESC`; the opaque
   cursor is tenant/workspace-bound and never supplies authority.
5. Notes controllers, advice, access resolver, and adapter beans are gated by
   `IdentityProfiles.PERSISTENT_IDENTITY`. The default profile still boots
   without a DataSource.
6. Optional note workspace headers match S-02: a supplied selector is rebound
   and mismatch returns 403; headerless lookup starts from actor memberships,
   probes member tenants under RLS, then re-binds the server-side workspace.
7. Soft-delete and purge return empty HTTP 204 responses. ProseMirror bodies
   must be JSON objects; titles and bodies are not logged.

### Verification

```text
cd apps/api && ./gradlew test --tests com.omnidoc.api.notes.NotesHttpIT
# BUILD SUCCESSFUL — 8 tests, 0 skipped/failures/errors

cd apps/api && ./gradlew clean build
# BUILD SUCCESSFUL — 30 tests, 0 skipped/failures/errors

git diff --check
# PASS
```

The suites cover fixture-rich CRUD, empty state, append-only history, stale and
concurrent version conflicts, tombstone visibility + purge, CSRF, optional
header resolution, same-workspace/tenant and cross-tenant IDOR negatives, RLS
zero visibility, and workspace-bound/malformed cursors.

No changes were made to `docs/api/openapi.yaml`, `packages/contracts`,
`docs/handoffs/current.md`, `lane-frontend.md`, mocks, frontend code, or B-05+.
