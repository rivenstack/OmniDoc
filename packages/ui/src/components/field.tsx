...
/**
 * Foundations / primitives — `Field` family (shadcn v4 `base-nova`, adapted).
 *
 * Source: `shadcn` registry. This is the v4 form pattern: forms compose
 * `FieldGroup` › `Field` › `FieldLabel` + control + `FieldDescription` /
 * `FieldError`. Never lay out raw `Label` + `Input` pairs with manual
 * spacing.
 *
 * Adaptations from upstream: logical spacing (`ms-*` instead of `ml-*`),
 * logical text alignment (`text-start`), and full-opacity focus rings.
 *
 * Accessibility contract:
 * - `Field` renders `role="group"` and `data-invalid`; set `data-invalid`
 *   on the field and `aria-invalid` on the control together.
 * - `FieldError` renders `role="alert"`, so a failed submit is announced.
 * - `FieldLabel` is a real `<label>`; keep `htmlFor` wired to the control.
 * - Required is carried by the control (`required` / `aria-required`), not
 *   by decoration.
 */
