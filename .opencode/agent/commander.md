...
- Treat `/ux-researcher` as the canonical role label; use the registered
  identifier `ux_researcher` in handoff metadata and `/ux-researcher`
  (alias `/ux_researcher`) in executable commands.
- Design/implementation MCP planning rule: when writing or updating
  ...
  visual assets. A vision-capable model in the session remains
  available for screenshot analysis. MCP output does not close
  architecture gates, OQs, or Phase Check. If an MCP is unauthenticated
  or missing from the live catalog, note auth/availability and continue
  with repo-native tools — do not block the handoff.
